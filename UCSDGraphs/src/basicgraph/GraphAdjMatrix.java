...
public class GraphAdjMatrix extends Graph {
	...
	public List<Integer> getDistance2(int v) {
		...
		int[][] sqrMatrix = new int[adjMatrix.length][adjMatrix.length];
				
		for (int i=0; i < adjMatrix.length; i++) {
			for (int j=0; j < adjMatrix.length; j++) {
				for (int k=0; k < adjMatrix.length; k++) {
					sqrMatrix[i][j] += adjMatrix[i][k] * adjMatrix[k][j];					
				}
			}
		}
		
		for (int i=0; i < getNumVertices(); i++) {
			for (int j=0; j < sqrMatrix[v][i]; j++) {
				if (sqrMatrix[v][i] > 0) {
					dist2.add(i);
				}
			}
		...
		//System.out.printf("Matrix distance from %d -> %s\n", v, dist2);
		return dist2;
	}
